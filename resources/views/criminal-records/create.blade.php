<x-layouts.app title="Add Criminal Record">
    <x-slot:header>
        <x-ui.page-header title="Add Criminal Record" :subtitle="$constituent->full_name">
            <x-slot:actions>
    ...
    <nav class="mb-4 text-sm text-slate-500" aria-label="Breadcrumb">
        ...
        <span class="px-1">/</span>
        <span class="text-slate-700">New criminal record</span>
    </nav>

    <form method="POST" action="{{ route('constituents.criminal-records.store', $constituent) }}" class="max-w-2xl">
        @csrf

        <x-ui.card title="Case details">
            @include('criminal-records._form', ['criminalRecord' => null])
        </x-ui.card>

        <div class="mt-5 flex items-center gap-3">
            <x-ui.button type="submit">Save Criminal Record</x-ui.button>
