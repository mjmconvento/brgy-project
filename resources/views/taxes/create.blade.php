<x-layouts.app title="Add Tax Record">
    <x-slot:header>
        <x-ui.page-header title="Add Tax Record" :subtitle="$constituent->full_name">
            <x-slot:actions>
    ...
    <nav class="mb-4 text-sm text-slate-500" aria-label="Breadcrumb">
        ...
        <span class="px-1">/</span>
        <span class="text-slate-700">New tax record</span>
    </nav>

    <form method="POST" action="{{ route('constituents.taxes.store', $constituent) }}" class="max-w-2xl">
        @csrf

        <x-ui.card title="Tax details">
            @include('taxes._form', ['tax' => null, 'months' => $months, 'statuses' => $statuses])
        </x-ui.card>

        <div class="mt-5 flex items-center gap-3">
            <x-ui.button type="submit">Save Tax Record</x-ui.button>
